# M2-T1（+T5）报告：解压回调链桥接 + 多档案编排/测试模式
...
T5 的能力随 T1 同架构落地（`archivePaths` 是向量、统计经 `CDecompressStat` 累加、错误分文件级/归档级聚合），增量验证用例：

```
F) 多档案批量   test.7z + test.zip 一次 run → 档案=2 文件=8 目录=6，统计正确累加
G) 批量含损坏   test.7z + broken.7z → 好档照解（文件=4 字节一致），坏档计入打开错误=1，互不影响
D) 测试模式     testMode 无错 → 输出 "There are no errors"（对齐 7zz t 总结文案）
```

→ 单档失败不中断批量（`ExtractResult` 仅在 `E_ABORT` 时中止，其余继续），与 7zz 多档案语义一致。损坏档各类**文件级**文案（CRC/DataError/UnexpectedEnd）的触发样本归 M2-T8 回归集（届时有真实损坏样本库），文案映射本身已在 T1 1:1 移植并经 E/G 的打开失败路径验证。

## 产物

- 新增：`Mac/SevenZipKit/src/SZExtractCore.{h,cpp}`、`Mac/SevenZipKit/tests/test_extract.cpp`、`Mac/SevenZipKit/build_test_extract.sh`（一键复现，含 A–G 七用例）。
