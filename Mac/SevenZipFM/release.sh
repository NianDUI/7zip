...
# 内核版本（上游 7-Zip）——从 C/7zVersion.h 自动读取，零维护：同步上游升级后自动跟变
CORE_VER="$(grep -m1 'MY_VERSION_NUMBERS' C/7zVersion.h | sed -E 's/.*"([^"]+)".*/\1/')"
echo "  内核 = 7-Zip ${CORE_VER:-未知}"
...
NOTES="$(mktemp)"
{
  [ -n "$CORE_VER" ] && printf '> 内核基于 7-Zip %s\n\n' "$CORE_VER"
